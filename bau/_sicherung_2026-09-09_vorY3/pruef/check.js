...
for (const k of kaps) {
  ...
  if (merke.length > 5) warn.push(`${k}: ${merke.length} Merke-Blöcke (höchstens 5)`);
  for (const b of merke) if (!/data-art="(Auf einen Blick|Merke|Typischer Fehler|Prüfungsnotiz)"/.test(b)) errs.push(`${k}: unerlaubter Merke-Typ ${b}`);
  const w = woerter(t);
  ...
  const erlaubt = new Set(['kapitel','kapitel-kopf','kapitel-nr','kam-dran','begriff','label','en','abb','l','f','w','a','fa','d','g','t2','tb','ta','mono','mitte','rechts','tabelle','z','summe','hervor','rechenweg','merke','aufgabe','herkunft','text','inhalt','q','quellenliste','typ','abruf','wo','selbstcheck','teil','unter','klein','glossar','kapitel-wege','begriffsliste']);
