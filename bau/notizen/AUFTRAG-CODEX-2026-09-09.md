# Auftrag – Lerndatei weiter verbessern (Stand 2026-09-09, 09:40 Uhr)

**Stand für den Nachfolger (Opus):** Codex hat Einheit 1 erledigt (70 Kern-Überschriften umbenannt, Protokoll unter
„Entscheidungen Y“ in ENTSCHEIDUNGEN.md) und dann sein Kontingent aufgebraucht. Fable hat Codex' Arbeit gegen die
Sicherung `_sicherung_2026-09-09_vorY/kapitel/` geprüft, fünf schiefe Umbenennungen korrigiert (a7-k11, a5-k8, a4-k1,
b8-k2, a5-k9) und Einheit 2 fertiggestellt (60 `lektion`-Felder in den `WEGWEISER-*.json`, Skript war von Codex
bereits erweitert). Koffer und Bau sind identisch und geprüft. **Du beginnst mit Einheit 3.**
Erster Schritt trotzdem: Abschnitt Y in ENTSCHEIDUNGEN.md lesen und einmal `node pruef/bauen-in-bau.js` sowie
`node pruef/abnahme.js` laufen lassen, damit du weißt, dass du auf grünem Stand startest. Die Sicherung aus Schritt 0
existiert schon; lege für deine Arbeit einen eigenen Ordner `_sicherung_2026-09-09_vorY3` an.
...
Sparsam arbeiten: Lies nicht alle 20 Kapitel am Stück. Lies je Einheit nur die Datei, die du gerade änderst,
und davon nur den Abschnitt (mit `grep -n` die Zeile finden, dann `sed -n 'a,bp'`). Für Einheit 1 liegt die
komplette Liste aller Kern-Überschriften schon in `notizen/Y1-UEBERSCHRIFTEN.md`; dort die Spalte „neu“
ausfüllen und die Umbenennung per Skript einspielen, statt Kapitel zu lesen. Für Einheit 2 reichen die
`WEGWEISER-<id>.json` plus die Überschriftenliste aus Einheit 1, um `lektion` zuzuordnen. Keine
Zwischenberichte, keine Zusammenfassungen des Gelesenen, keine Pläne ausformulieren: machen, prüfen, loggen.
