...
function woerter(t) {
  return t.replace(/<svg[\s\S]*?<\/svg>/g, ' ').replace(/<[^>]+>/g, ' ').replace(/&[a-z#0-9]+;/g, ' ').split(/\s+/).filter(Boolean).length;
}
for (const k of kaps) {
  const t = kapText(k);
  for (const teil of TEILE) if (!t.includes(`id="${k}-${teil}"`)) errs.push(`${k}: Abschnitt "${teil}" fehlt`);
  if (!t.includes(`data-key="${k}"`)) errs.push(`${k}: Checkbox "Sitzt" fehlt`);
  ...
  for (const karte of t.match(/<dl class="begriff"[\s\S]*?<\/dl>/g) || []) {
    ...
    const soll = ['Theorie', 'Praxis', 'Merkhilfe und Verwechslung', 'In der Prüfung'];
    if (labels.join('|') !== soll.join('|')) {
      const dt = (karte.match(/<dt>([\s\S]*?)<\/dt>/) || [, '?'])[1].replace(/<[^>]+>/g, '');
      errs.push(`${k}: Begriffskarte "${dt.trim()}" hat nicht die vier Zeilen Theorie/Praxis/Merkhilfe und Verwechslung/In der Prüfung`);
    }
    if (!/<span class="label">Theorie<\/span>[\s\S]*?<sup class="q">/.test(karte.split('<span class="label">Praxis')[0])) {
      const dt = (karte.match(/<dt>([\s\S]*?)<\/dt>/) || [, '?'])[1].replace(/<[^>]+>/g, '');
      errs.push(`${k}: Begriffskarte "${dt.trim()}": Theorie-Zeile ohne Quellenverweis`);
    }
  }
  ...
  if (merke.length > 4) warn.push(`${k}: ${merke.length} Merke-Blöcke (höchstens 4)`);
  for (const b of merke) if (!/data-art="(Merke|Typischer Fehler|Prüfungsnotiz)"/.test(b)) errs.push(`${k}: unerlaubter Merke-Typ ${b}`);
  const w = woerter(t);
  if (w > 4500) errs.push(`${k}: ${w} Wörter, Obergrenze 4500`);
  ...
  const erlaubt = new Set(['kapitel','kapitel-kopf','kapitel-nr','kam-dran','sitzt','begriff','label','en','abb','l','f','w','a','fa','d','g','t2','tb','ta','mono','mitte','rechts','tabelle','z','summe','hervor','rechenweg','merke','aufgabe','herkunft','text','inhalt','q','quellenliste','typ','abruf','wo','selbstcheck','teil','unter','klein','glossar']);
