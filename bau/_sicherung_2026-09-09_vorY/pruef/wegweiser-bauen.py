...
for jf in sorted(glob.glob(os.path.join(N, "WEGWEISER-*.json"))):
    ...
    for e in eintraege:
        ...
        zeilen.append((REIHE.index(key[0]), key[1], key, text, e["ziel"]))
    for key in soll - ist:
        fehler.append(f"{kid}: Aufgabe {key} fehlt im Wegweiser")
    zeilen.sort(key=lambda z: (z[0], [int(x) for x in z[1].split(".")]))
    # Tabelle bauen
    tr = "".join(f'\n    <tr><td>{NAMEN[k[0]]}</td><td>{k[1]}</td><td>{html.escape(titel[k])}</td><td><a href="#{z}">{html.escape(t)}</a></td></tr>'
                 for _, _, k, t, z in zeilen)
    tabelle = ('\n  <nav aria-label="Prüfungsaufgaben dieses Kapitels"><div class="tabelle"><table>'
               '\n    <caption>Prüfungsaufgaben zu diesem Kapitel und wo die Antwort steht</caption>'
               '\n    <tr><th>Prüfung</th><th>Aufgabe</th><th>Worum es ging</th><th>Hier steht es</th></tr>' + tr +
               '\n  </table></div></nav>\n')
    m = re.search(r'(<section id="%s-worum">[\s\S]*?)(\n\s*<nav aria-label="Prüfungsaufgaben dieses Kapitels">[\s\S]*?</nav>\n)?(</section>)' % kid, s)
    if not m:
