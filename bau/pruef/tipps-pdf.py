...
for code, _, _ in PRUEF:
    ...
    # Zeilenform seit dem Umbau der Prüfungsansicht am 2026-09-09: <tr id= data-weg=>,
    # Kapitelkürzel im <span class="kap">, dahinter ein oder zwei absatzgenaue Sprungziele.
    for m in re.finditer(r'<tr[^>]*><td>([1-4]\.\d+)</td><td>(.*?)</td><td>(.*?)</td><td class="z">(\d+)</td><td><span class="kap">([AB]\d+)</span>', seg):
