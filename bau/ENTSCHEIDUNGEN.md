...
## AC — Tipp-Skript wieder lauffaehig (2026-09-10, Opus)

**Befund.** `pruef/tipps-pdf.py` liest in Abschnitt 1 die Kapitelzuordnung aus `kapitel/03-matrix.html`
mit einer festen Regex, die die alte Zeilenform erwartete: `<tr><td>` ohne Attribute und in der letzten
Zelle direkt `<td><a href="#a1">`. Mit dem Umbau der Pruefungsansicht (Abschnitt AB, 2026-09-09) hat jede
Zeile eine `id` und `data-weg`, und das Kapitelkuerzel steht in `<span class="kap">`, gefolgt von einem
oder zwei absatzgenauen Zielen. Die Regex traf danach in allen neun Pruefungen 0 von 252 Zeilen. Folge:
`mx` blieb leer, jede Teilaufgabe meldete "fehlt in 03-matrix.html", das Skript brach mit Exit 1 ab. Die
Papierseite des Koffers liess sich seit dem 2026-09-08 nicht mehr neu bauen.

**Geaendert.** Eine Zeile in `pruef/tipps-pdf.py`: Regex auf `<tr[^>]*>` geoeffnet und das Kapitel aus
`<span class="kap">([AB]\d+)</span>` statt aus dem href genommen. Das Sprungziel wird hier nicht gebraucht,
die Matrix dient in diesem Skript nur der Gegenprobe von Kapitel und Punkten. Damit ist die Regex auch
gegen kuenftige Aenderungen am Sprungziel unempfindlich. Kein weiterer Eingriff.

**Geprueft.** Alte Regex 0 Treffer, neue 252. Vollstaendiger Lauf gegen
`_sicherung_2026-09-08_pdf/AP1_Pruefungen_2021-2025_ORIGINAL-112-Seiten.pdf`: 252 Teilaufgaben, alle neun
Pruefungen 100 Punkte, keine Fehlermeldung, 105 Seiten, Lesezeichen [1, 12, 26, 39, 48, 60, 69, 79, 91].
`notizen/TIPPS-ZUORDNUNG.json` wieder 252 Eintraege und 900 Punkte. Seitenweiser Textvergleich des neuen
PDF gegen `AP1-Koffer/AP1_Pruefungen_2021-2025.pdf`: 105 zu 105 Seiten, keine einzige Seite mit
abweichendem Text.

**Wichtig fuer die Einordnung.** Das Papier war inhaltlich nicht veraltet. Die Randkaesten entstehen aus
den Notizen (`notizen/20NN-*.md`, Heftbezeichnung und Kapitel), nicht aus der Matrix; die Matrix war nur
die Gegenprobe. Kaputt war also die Moeglichkeit, neu zu bauen, nicht das gebaute Ergebnis. Der Koffer
wurde deshalb nicht angefasst. Die wiederhergestellte Gegenprobe bestaetigt jetzt, dass Notizen und
Matrix bei allen 252 Teilaufgaben in Kapitel und Punkten uebereinstimmen.

**Was offen bleibt.** Die Kastenzeile lautet weiterhin `heft » KAPITEL Kurzname`, also z. B. "c) » A6
Netzplan/Gantt". Die Nummer der Lerndatei (1.3) und der Abschnittstitel stehen nicht darauf; der Weg vom
Papier in die Datei traegt damit nur das Kapitel, nicht den Absatz. Das ist ein eigener Auftrag.

## AD — RAID 1: Nutzkapazitaet berichtigt (2026-09-10, Opus)

**Befund.** Das Rechenweg-Schema in B7 und die Uebersicht Abb. B7-6 rechnen ausdruecklich mit acht
Platten zu 4 TB ("Bei 8 Platten zu 4 TB", Bildunterschrift "gerechnet mit acht Platten zu 4 TB"). In der
Zeile RAID 1 stand `K je Paar   1 × 4 TB   =  4 TB`. Acht Platten sind vier Spiegelpaare, also
4 × 4 TB = 16 TB. Die Zeile widersprach damit dreifach: der eigenen Formel "K je Paar", der Zeile
RAID 10 darunter (gleiche Plattenzahl, 16 TB) und dem Merke-Kasten weiter oben ("RAID 1 spiegelt und
kostet die Haelfte des Platzes", also die Haelfte von 32 TB). Vor allem widersprach sie der geloesten
Uebung im selben Kapitel: "RAID 1 als zwei Spiegelpaare: 2 × 6 TB = 12 TB" bei vier Platten zu 6 TB.
Die Zeile sprang stillschweigend von n = 8 auf ein einzelnes Paar.

**Geaendert.** Drei Stellen, alle auf die Konvention des Kapitels gebracht ((n / 2) × K):
`kapitel/26-b7.html` Rechenweg-Schema (`(n / 2) × K   (8 / 2) × 4 TB = 16 TB`), Abb. B7-6 zwei
Textknoten (Formelzelle `K je Paar` -> `(n / 2) × K`, Wertzelle `4 TB` -> `16 TB`), und
`kapitel/90-verfahrensblatt.html` Zeile 178, wo dasselbe Schema ein zweites Mal steht. Das
Verfahrensblatt war beim ersten Durchgang uebersehen worden und fiel erst bei der Trefferzaehlung im
gebauten Dokument auf (3 statt 2 Treffer).

**Nicht geaendert.** Abb. B7-5 zeigt `1 × 4 TB = 4 TB` weiterhin richtig: sie ist ausdruecklich mit
"RAID 1 · zwei Platten zu je 4 TB" ueberschrieben, dort ist n = 2. Ebenso bleibt der Fliesstext
"RAID 1 nutzt je Paar den Platz einer Platte" stehen; er erklaert, woher (n / 2) × K kommt.

**Bildunterschrift.** Weil RAID 1 und RAID 10 in der Tabelle jetzt zweimal 16 TB zeigen und das wie ein
Abschreibfehler aussehen kann, endet die Unterschrift von Abb. B7-6 auf "RAID 1 und RAID 10 sind hier
gleich gross." Der Unterschied steht schon im Absatz darueber (RAID 10 streift zusaetzlich, ist also
schneller). Der erste, laengere Satz dazu (19 Woerter) hat B7 auf 4503 Woerter und damit ueber die
Obergrenze 4500 gehoben; check.js meldete das als FEHLER. Auf neun Woerter gekuerzt, B7 steht jetzt bei
4492.

**Geprueft.** `node pruef/bauen-in-bau.js`, `node pruef/check.js AP1_Lerndatei.html` 0 Fehler und
dieselben 17 Hinweise wie vorher, `node pruef/abnahme.js` "Keine Befunde.", `python pruef/vorschau.py b7`
7 Abbildungen, "Keine Befunde." Streifen b7-teil5.png angesehen: Tabelle liest sich
n × K / (n / 2) × K / (n − 1) × K / (n / 2) × K und 32 / 16 / 28 / 16 TB. `grep 'K je Paar'` im gebauten
Dokument 0 Treffer. Koffer mit genau drei Dateien aktualisiert.

## AE — Druck: aufgeklappte Klappen waren weiss auf weiss (2026-09-11, Opus)

**Befund.** `beforeprint` klappt vor dem Druck alle 291 `details` auf (Skript in `kapitel/99-fuss.html`).
Fuer aufgeklappte Klappen gilt `kapitel/00-kopf.html` Zeile 142
`details[open] summary{background:var(--tinte);color:var(--papier)}`, also weisse Schrift auf dunkler
Pille, und Zeile 145 setzt die Minus-Marke ebenfalls auf `var(--papier)`. Der Druckblock wollte das mit
`summary{color:inherit}` zurueckdrehen, ist aber schwaecher: Spezifitaet 0-0-1 gegen 0-1-2. Die Schrift
blieb also weiss. Browser drucken Hintergruende standardmaessig nicht ("Hintergrundgrafiken" ist im
Druckdialog von Chrome und Edge nicht angehakt), damit verschwindet die dunkle Pille und die weisse
Schrift steht auf weissem Papier. Betroffen war jede Beschriftung "Antwort", "Loesung mit Punktelogik",
jeder Begriffsname und jede Wegweiser-Zeile. Mit angehakten Hintergrundgrafiken war der Druck lesbar;
darum fiel es bei der Sichtpruefung nicht auf. Befund stammt aus dem Gutachten vom 2026-09-10.

**Geaendert.** Zwei Zeilen im Druckblock von `kapitel/00-kopf.html`, direkt hinter
`summary{color:inherit}` und damit mit gleicher Spezifitaet, aber spaeter als Zeile 142:
`details[open] summary{background:#fff;color:#000}` und
`details[open] summary::before{color:#000}`. Jetzt ist der Druck in beiden Faellen richtig: ohne
Hintergrundgrafiken schwarze Schrift auf Papier, mit Hintergrundgrafiken schwarze Schrift auf weisser
Pille mit dunklem Rand. Die geschlossene Klappe war schon vorher richtig (Zeile 140 setzt
`background:var(--papier)` und `color:var(--tinte)`).

**Nicht geaendert.** Zeilen 142 und 145 bleiben, wie sie sind. Am Bildschirm soll die aufgeklappte
Pille weiterhin dunkel mit weisser Schrift sein; das ist die Anzeige, dass sie offen ist.

**Gemessen, nicht geschaetzt.** Zwei Druckproben mit Headless Edge aus je einer Kopie mit allen 291
Klappen offen (`--print-to-pdf`, 474 Seiten), Textfarbe je Textstueck mit PyMuPDF ausgelesen. Von 240
gefundenen Klapp-Beschriftungen waren vorher 195 in reinem Weiss (#FFFFFF), nachher 0; alle stehen auf
#000000. Seite 49 beider Proben als PNG angesehen: die Pille "ANTWORT" unter "Frage vorab" in A1 ist
vorher weiss auf schwarz, nachher schwarz auf weiss mit Rand. Die restlichen 45 Treffer waren das Wort
"Loesung" im Fliesstext, nie weiss.

**Geprueft.** `node pruef/bauen-in-bau.js` 28 Teile, 1384 KB. `node pruef/check.js AP1_Lerndatei.html`
0 Fehler und dieselben 17 Hinweise wie vorher. `node pruef/abnahme.js` "Keine Befunde." Koffer mit genau
drei Dateien aktualisiert. Am Inhalt hat sich nichts geaendert, nur eine Druckregel, darum keine
Kapitelvorschau.

**Nebenbefund, nichts zu tun.** Der Hinweis "Tages-/Wochenplan (verboten) gefunden" aus check.js ist ein
Fehlalarm. Die Regel in Zeile 157 sucht `\b(Tag|Woche)\s+\d+\b` und trifft die Netzplan-Termine "am
Tag 12" und "am Tag 9" in A6. Das sind echte Termine, kein Lernplan.

## AF — Struktogramm und Programmablaufplan in A7 (2026-09-11, Opus)

**Befund.** A7 kuendigte im Einstieg an, die Pruefung verlange "Struktogramme ergaenzen", und der Wegweiser
fuehrte zwei Aufgaben darauf: F22 4.4 (9 P, verschachtelte Schleifen fuellen) und H22 4.4 (7 P,
Produktionsauftrag mit Notaus ergaenzen). Die Notation selbst stand nirgends in der Datei. Zwei Stellen in
A7 verwiesen dafuer auf A8 ("Zur Diagrammform siehe A8"): einmal im Abschnitt ueber verschachtelte
Schleifen, einmal in der Begriffskarte Pseudocode. A8 enthaelt Anwendungsfall-, Aktivitaets- und
Klassendiagramm und kein einziges Vorkommen von "Struktogramm" oder "Programmablaufplan". Beide Verweise
waren also tot. Befund stammt aus dem Gutachten vom 2026-09-10.

**Wo es hingehoert.** Nach A7, nicht nach A8. Ein Struktogramm ist keine UML-Form, sondern DIN 66261;
A8 heisst "UML-Diagramme". A7 heisst "Programmlogik und Schreibtischtest", und die beiden Pruefungsaufgaben
sind im Wegweiser bereits A7 zugeordnet. A8 umzubenennen haette Navigation, MATRIX und Teil 2 mitgezogen.

**Neu.** Abschnitt `a7-k13` "Struktogramm: der Ablauf als verschachtelte Kaesten" hinter dem Abschnitt
ueber verschachtelte Schleifen, damit er auf Abb. A7-4 zurueckgreifen kann – die ist bereits in dieser Form
gezeichnet, hiess aber nie so. Inhalt: Frage vorab mit Antwort, die vier Sinnbilder, Leseregel von oben nach
unten und von aussen nach innen, Abgrenzung zum Programmablaufplan (DIN 66001: Oval, Rechteck, Raute,
Pfeile), und was die Pruefung frei laesst. Dazu Abb. A7-7 mit allen vier Formen; im Akzent die Zeile im
Schleifenkopf, weil die in den Heften am haeufigsten leer ist. A7 hat damit 7 Abbildungen, die Datei 132 in
den Kapiteln.

**Quellen.** `din-66261` (DIN 66261:1985-11, Sinnbilder fuer Struktogramme nach Nassi-Shneiderman) und
`din-66001` (DIN 66001:1983-12, Sinnbilder und ihre Anwendung) als `a7-q11` und `a7-q12`. Beide hinter
Bezahlschranke, daher ohne URL und mit "(nicht online geprueft)", wie bei `din-69900` in A6. Beide in
`notizen/QUELLEN-A.md` eingetragen, sonst meldet abnahme.js den Schluessel als unbekannt (genau das ist
beim ersten Lauf passiert).

**Wegweiser.** F22 4.4 und H22 4.4 zeigen in `notizen/WEGWEISER-a7.json` jetzt auf `a7-k13` statt auf
`a7-k8`, mit dem Hinweis auf Abb. A7-4 fuer die Verschachtelung. `python pruef/wegweiser-bauen.py` neu
gelaufen, 252 Aufgabenzeilen in Teil 2 wieder gesetzt. Die beiden toten A8-Verweise zeigen jetzt auf
`a7-k13`; `grep 'Diagrammform[^<]*<a href="#a8"'` im gebauten Dokument 0 Treffer.

**Wortgrenze: Ausnahme fuer A7.** Mit dem neuen Abschnitt steht A7 bei 4675 Woertern, die Obergrenze war
4500. Drei Stellen habe ich zuerst gekuerzt (zusammen 55 Woerter): den ueberholten Halbsatz "So sieht auch
ein Struktogramm aus", den Kasten `data-art="Prüfungsnotiz"` (sagte dasselbe wie der Kasten "Für die
Prüfung reicht" im selben Kapitel) und den Satz ueber die drei Schleifenformen in Baustein 4, dessen
Kopf-/Fuss-Unterscheidung jetzt im neuen Abschnitt steht. Danach fehlten noch 174 Woerter. Alle neun
Begriffskarten liegen bei 71 bis 101 Woertern und damit im Budget (60 bis 120); die Lektionsabschnitte
tragen jede Zeile. Weiterschneiden haette Inhalt gekostet, nicht Fuell-Text. Der Lernende hat daher
entschieden, die Grenze fuer A7 allein auf 4700 zu setzen (`pruef/check.js`, `const grenze`), alle anderen
Kapitel bleiben bei 4500. Begruendung: die Grenze soll Auffuellen verhindern, hier kommt ein fehlendes
Pflichtthema dazu. BAUPLAN.md §10 nennt weiterhin 4500 und wurde nicht angefasst (Eiserne Regel 11:
nichts ausserhalb von `Lernen/bau/` schreiben). Nebenbefund: der Lektionsteil von A7 hat rund 2170
Woerter, das Teilbudget in §10 nennt 500 bis 1000. Das Kapitel war schon vor dieser Aenderung ueber dem
Teilbudget.

**Zwei Textkorrekturen nach der Bildpruefung.** Im Bild stand "lies naechsten Wert" in Prosa (kein
Variablenname), korrigiert auf "nächsten". Die Bildunterschrift endete auf "der der fußgesteuerten
darüber", umgestellt auf "bei der fußgesteuerten darüber".

**Geprueft.** `node pruef/check.js kapitel/16-a7.html --fragment` "Keine Fehler." `node
pruef/bauen-in-bau.js` 28 Teile, 1388 KB, 195 Quellen. `node pruef/check.js AP1_Lerndatei.html` 0 Fehler,
dieselben 17 Hinweise wie vorher (a7 steht jetzt mit 4675 im Richtwert-Hinweis). `node pruef/abnahme.js`
"Keine Befunde." `python pruef/svgcheck.py` 0 Befunde ueber alle Abbildungen. `python pruef/vorschau.py a7`
